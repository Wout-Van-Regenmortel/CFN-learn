...
num_validations = 32 #komt van 1024
# validation_set = os.path.join("Sudoku",os.path.join("validation-set","rrn-validation.csv.xz"))
validation_set_path = '/home/wout/CFN-learn/Other/NQueens/ValidationSets/validation8x8/instance.json'
    ...
    for idx in range(n_lambdas):
        ...
        for s,hint in enumerate(valid_hints[:num_val]):
            ...
            if (sol):
                ...
                pgridLatin(mode,ltruth,lhint,num_nodes, size_block, list(sol[0]), MNIST_decode(valid_sols[s].strip(), s) if mode > 0 else None)
                if (mode < 2):
