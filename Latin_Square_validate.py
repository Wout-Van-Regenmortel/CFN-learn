...
num_validations = 512 #komt van 1024
# validation_set = os.path.join("Sudoku",os.path.join("validation-set","rrn-validation.csv.xz"))
validation_set_path = '/home/wout/CFN-learn/Other/LatinSquare/ValidationSets/validation4x4/instance.json'
with open(validation_set_path, "r") as validation_set:
    dict_validationset = json.load(validation_set)

list_data = list()
for square_dict in dict_validationset['solutions']:
...
A_matrix_path = '/home/wout/CFN-learn/Other/LatinSquare/Amatrices/Amatrix.csv'
...
while (num_val <= num_validations):
    ...
    for idx in range(n_lambdas):
        ...
        for s,hint in enumerate(valid_hints[:num_val]):
            ...
            if (sol):
                pgridLatin(mode,ltruth,lhint,num_nodes, list(sol[0]), MNIST_decode(valid_sols[s].strip(), s) if mode > 0 else None)
                if (mode < 2):
